...
| └ `required_approving_review_count` | `0` | approval 数は要求しない（品質は thread 解決で担保。owner 主導の repo のため） |
...
つまり **「approval N 件必須」を merge 条件にすると、agent 単独運用が構造的に詰む**。本 repo が `required_approving_review_count: 0` にしているのはこの帰結で、品質は **ローカル codex review（[../CLAUDE.md](../CLAUDE.md) Step 4）+ review thread resolution** で担保する。「人間 approval を必須 gate にする」方向と agent 自律運用は両立しないと理解しておく。
