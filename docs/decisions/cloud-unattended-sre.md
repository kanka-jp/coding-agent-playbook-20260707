...
**ステータス: Proposed（2026-06-29）** — 実行基盤（Amazon Bedrock）・認証（AWS IAM）・2 パターン（`claude -p` / Agent SDK）の設計は確定。実機 spike（実デプロイ検証）は未通過で、通過後に Accepted へ更新する。
...
CloudWatch (ALB/API GW 5XX alarm  or  Logs metric filter ERROR/5xx)
   └─► Alarm ─► SNS ─► Lambda(trigger + triage) … 観測 identity（AWS read + 一方向 dispatch のみ・外部 egress なし）
...
**identity 境界（決定・拘束条件）**: ログ / AWS の read は **Lambda の triage 段（観測 identity）でのみ**行う。観測 identity の権限は **AWS read + fixer を起動する一方向 dispatch（`StartBuild` / `RunTask` / queue publish 等）だけ**に絞り、**GitHub を含む外部 egress は持たせない**。ただし **dispatch の payload 自体が間接 exfil チャネルになり得る**: 観測側がログ由来データを env override / message body に自由に載せられると、fixer の GitHub egress 経由で抜けて trifecta が復活する。よって **handoff する triage は固定 schema・allowlist・サイズ制限・raw log/secret 禁止・override 禁止に拘束し、dispatch 境界自体を sanitize gate とする**。パターン A/B の fixer は **sanitized な triage 結果 + repo だけ**を入力に取り、**AWS read を持たない別 identity**とする。単一 identity（単一 Fargate/AgentCore role）に CloudWatch read と GitHub write を同居させてはならない（破ると本 ADR が避ける lethal trifecta が成立する。下記「安全」参照）。
...
- **観測（AWS read + 一方向 dispatch のみ・GitHub/外部 egress なし）と 修正（repo-scoped・AWS read を持たない）を別 identity に分離**する。
...
- **実機 spike 未通過**: パターン A（Fargate/CodeBuild での `claude -p` on Bedrock）/ B（AgentCore Runtime での Agent SDK）の実デプロイ検証、CloudWatch→Lambda 配線、approval gate の Slack 連携は後続。通過後に Accepted へ。
