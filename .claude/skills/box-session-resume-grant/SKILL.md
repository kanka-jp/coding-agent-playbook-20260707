...
**(a) bridge dir 確保 + 旧 sentinel 予防削除** (compound `&&` は host hook で許可):
...
**(c) done sentinel touch**:

```bash
touch <repo-root>/.claude/host-bridge/resume-ans-<box-name>-<seq>.md.done
```

**(a)→(b)→(c) の順序必須**。逆順だと box 側 polling が古い/半書きの body を早期 cat する race が再発する。reject 経路でも (b) で reject 用の ans body (下記 format で `exit: rejected`) を Write して (c) で sentinel touch する (box Monitor 解放のため必須)。
