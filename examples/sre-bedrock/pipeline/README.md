...
| 題材アプリ（仕込みバグ込み） | stage 系（既存 `stage/06`→`07` の drift 系を流用可） |

## build order（次の一手）

1. **A の最小 e2e**: [`infra/`](infra/) の CDK を host で deploy → S3 に triage を置き → CodeBuild で fixer の `claude -p`（直 key）を回し → PR が出るところまで（spike の延長。直 key なので Bedrock 承認待ち不要）。← IaC done、deploy は host。
2. **triage 配線**: CloudWatch 5XX alarm → SNS → Lambda（Logs Insights → actionable 判定 → sanitize → S3 PUT）。起動は S3 event → EventBridge rule → StartBuild/RunTask を別に配線（観測に StartBuild を持たせない）。
3. **identity 締め**: 2 ポリシーを実 role に当て、観測が Bedrock/secret 不可・修正が incident read 不可を実機で確認。
4. **B を併設**して A と比較 → ADR 段階 4（教材化）。

各段が通るごとに ADR「残差・未決」を消し、安定したら ADR を `Proposed → Accepted` に更新する。
