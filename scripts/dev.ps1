...
# 'route' publishes a box web preview as web.<name>.localhost via a Traefik routing layer (verbs: up / add / rm / ls / down / detect).
#
...
# name = <branch>.<repo>; other subcommands ignore it.
...
usage:
  ...
  pwsh scripts/dev.ps1 route <verb> [args]   publish web preview as web.<name>.localhost via Traefik
...
# ---------------- route subcommand: publish web preview as web.<name>.localhost via Traefik ----------------
# Routing layer for parallel box (dev / sandbox) web previews. Box create/run is handled by dev.ps1 itself
# (Start-DevBox / Invoke-Sandbox); the route subcommand only touches the routing layer (separated from box lifecycle).
...
function Get-RouteStoreNames { param($Dynvol, $Dyn)
  if ($Dynvol) { docker run --rm -v "${Dynvol}:/d" $RouteStoreImg sh -c 'for f in /d/*.yml; do [ -e "$f" ] && basename "$f" .yml; done' 2>$null }
  else { Get-ChildItem -LiteralPath $Dyn -Filter *.yml -ErrorAction SilentlyContinue | ForEach-Object { $_.BaseName } }
}
...
usage: pwsh scripts/dev.ps1 route <verb> [args]
  ...
  add <box> [port] [name]     publish box dev port to an ephemeral host port and add web.<name>.localhost route
                              name default = <branch>.<repo> (from checkout); dot-separated DNS labels allowed
  ...
  switch ($Verb) {
    "up" {
      ...
      if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "  - No named URL needed: sbx ports <box> --publish <port>:<port> then open http://localhost:<port>."
        exit 1
      }
      Write-Host "Traefik up. Routed boxes are reachable at web.<name>.localhost."
    }
    "add" {
      ...
      # name default = <branch>.<repo> (from current checkout) -> web.<branch>.<repo>.localhost
      ...
      if (-not $Name) {
        ...
        if ($branch) { $Name = "$branch.$repo" } else { $Name = $repo }
      }
      ...
      if ($g.rc -eq 0) {
        ...
        if ($prev -ne $Box) { Write-Error "route '$Name' already exists (box='$prev'; markerless=possibly foreign); not overwriting. Use an explicit name: pwsh scripts/dev.ps1 route add $Box $Port <unique.name>"; exit 1 }
      }
...
"@
      ...
      if ($piggyback) { $tag = if ($detectSrc -eq "auto") { "piggyback/auto" } else { "piggyback" }; Write-Host "routed: http://web.$Name.localhost -> host:$hostport -> ${Box}:${Port} ($tag)" }
      else { Write-Host "routed: http://web.$Name.localhost -> host:$hostport -> ${Box}:${Port}" }
    }
    "rm" {
    ...
    "ls" {
      $names = Get-RouteStoreNames -Dynvol $dynvol -Dyn $dyn
      if ($names) {
        foreach ($n in $names) {
          $c = Read-RouteStoreEntry -Name $n -Dynvol $dynvol -Dyn $dyn
          $u = ""
          if ($c) { $um = [regex]::Match($c, 'url:\s*"([^"]+)"'); if ($um.Success) { $u = $um.Groups[1].Value } }
          Write-Host "http://web.$n.localhost -> $u"
        }
      } else {
