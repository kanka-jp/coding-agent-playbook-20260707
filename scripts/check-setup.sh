...
# 8. stage worktree 展開済み (warn 扱い: 任意。stage を試したい時だけ必要)
if [ -d .worktrees ] && find .worktrees -mindepth 1 -maxdepth 1 -type d -print -quit 2>/dev/null | grep -q .; then
  count=$(find .worktrees -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
  ok "stage worktrees expanded ($count dir(s) under .worktrees/)"
else
  warn "stage worktrees 未展開" "bash scripts/internal/setup-worktrees.sh で展開 (stage を使う時に必要)"
fi
