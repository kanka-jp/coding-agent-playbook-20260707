...
# route <verb> [args] (Traefik で web preview を web.<name>.localhost で公開)。
#
# lifecycle 不変条件:
...
# name = <branch>.<repo>) が壊れる)。route 以外の subcommand では未使用。
...
usage:
  ...
  bash scripts/dev.sh route <verb> [args]   Traefik で web preview を web.<name>.localhost で公開
...
# ---------------- route subcommand: Traefik で web preview を web.<name>.localhost で公開 ----------------
# 並列 box (dev / sandbox) の web preview を名前 (web.<name>.localhost) でルーティングする host 側機能。
...
_route_store_list() { # -> name 一覧 (.yml を除いた basename)
  if [ -n "$dynvol" ]; then
    docker run --rm -v "$dynvol":/d "$_route_store_img" sh -c 'for f in /d/*.yml; do [ -e "$f" ] && basename "$f" .yml; done' 2>/dev/null
  else
...
usage: bash scripts/dev.sh route <verb> [args]
  ...
  add <box> [port] [name]     box の dev port (default 3000) を ephemeral host port で publish し
                              web.<name>.localhost に経路を追加 (Traefik hot-reload)
                              name 既定 = <branch>.<repo> (現 checkout 由来) -> web.<branch>.<repo>.localhost
                              name はドット区切りの DNS ラベル列を許可 (例 04-mvp.coding-agent-playbook)
  rm <name>                   <name> の経路を削除
  ls                          現在の経路一覧
  ...
  case "$verb" in
    up)
      ...
      if docker compose -f "$_route_compose" up -d; then
        echo "Traefik up。add した box が web.<name>.localhost で見えます。"
      else
        echo "" >&2
        ...
        echo "  - 名前付き URL が不要なら: sbx ports <box> --publish <port>:<port> で http://localhost:<port> を直接開く" >&2
        exit 1
      fi
      ;;
    add)
      ...
      local port=${2:-3000}
      # name 既定は <branch>.<repo> (呼び出し元 checkout 由来) → web.<branch>.<repo>.localhost。
...
EOF
      if [ "$piggyback" = 1 ]; then
        ...
        echo "routed: http://web.$name.localhost -> host:$hostport -> $box:$port ($tag)"
      else
        echo "routed: http://web.$name.localhost -> host:$hostport -> $box:$port"
      fi
      ;;
    rm)
    ...
    ls)
      ...
      for n in $(_route_store_list); do
        found=1
        u=$(_route_store_read "$n" | awk -F'"' '/url:/ {print $2}')
        echo "http://web.$n.localhost -> $u"
      done
      [ "$found" = 1 ] || echo "(経路なし)"
      ;;
    down)
...
start_box() {
  ...
  if [ "$pair_ready" = 1 ]; then
    ...
    trap '
      ...
      bash scripts/internal/a2a-review.sh pair-teardown "'"$NAME"'" >/dev/null 2>&1 || true
      rm -f "'"$LOCK_FILE"'" "'"$PAIR_SERVE_LOG"'" 2>/dev/null || true
    ' EXIT INT TERM
  fi
