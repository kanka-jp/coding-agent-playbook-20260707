...
# README section 1 setup (sbx CLI / Docker / secret / image / stage worktree) verifier.
...
# 7. stage worktrees expanded (warn level; only needed when you actually want to open a stage)
$worktreesPath = Join-Path -Path (Get-Location) -ChildPath '.worktrees'
if (Test-Path -LiteralPath $worktreesPath -PathType Container) {
  $dirs = @(Get-ChildItem -LiteralPath $worktreesPath -Directory -Force -ErrorAction SilentlyContinue)
  if ($dirs.Count -gt 0) {
    Write-Ok "stage worktrees expanded ($($dirs.Count) dir(s) under .worktrees/)"
  } else {
    Write-Warn "stage worktrees not expanded" "Run: pwsh scripts/internal/setup-worktrees.ps1 (needed only when you want to open a stage)"
  }
} else {
  Write-Warn "stage worktrees not expanded" "Run: pwsh scripts/internal/setup-worktrees.ps1 (needed only when you want to open a stage)"
}
