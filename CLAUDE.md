...
講義用リポジトリ。全体像と運用モデルは [README.md](README.md) 参照。本ファイルは coding agent が作業する際の**高レベルの原則と開発フロー**を定め、詳細は `rules/` に委譲する（[rules/box-ops.md](rules/box-ops.md) / [rules/box-personas.md](rules/box-personas.md) / [rules/worktrees.md](rules/worktrees.md) / [rules/pr-followup.md](rules/pr-followup.md) / [rules/skills.md](rules/skills.md)）。
...
- **`stage/*` ブランチ = project 本体**。main と履歴を共有しない orphan 系列で、`.worktrees/<NN-slug>/` の worktree として展開して扱う（規約は下記「stage ブランチの規約」。stage は dev flow ではなく**講義の checkpoint 機構**）

## 作業場所のルール

- 講義資料（README / CLAUDE.md / rules/ / scripts/ / slides/）や `sbx/` の変更は作業用 worktree で行う（下記「コミット / PR 運用」）。project（demo アプリ）本体の実装は stage worktree で行い、**main checkout に project コードを置かない**。
- worktree の切り方・片付けは [rules/worktrees.md](rules/worktrees.md)、stage の規約・展開は下記「stage ブランチの規約」参照。
...
- **実装 done から PR 後続まで、中間確認なしで一気通貫**: 編集が done になったら、以下の chain を **確認を待たず連続して** 進める（global Autonomy / proceed-first の本リポでの具体化）。「PR 化しますか？」「次どうしますか？」「`/pr-codex-ci` を回しますか？」「このまま進めますか？」等の**選択肢提示で停止しない**（merge と、自走不能事象の HOTL escalate、および user が明示的に指定した停止点だけが停止点。user が「commit だけで止めて」「push しないで」等を明示している場合はそれに従う）。
    ...
    3. `gh pr create --base <base-branch> --title "<件名>" --body "$(cat <<'EOF' ... EOF)"`（**`--title` / `--body` 必須**。省略すると editor / 対話 prompt が開き hang する。`--base` も明示（省略時は default branch が選ばれ、stage worktree の PR が `main` に向く等の事故になる。本リポでは stage worktree からの PR は対応する前 stage を base にする）。issue 紐付きなら `Closes #N`、PR Body フッター必須）
...
`stage/*` は「講義中に『ここまで進んだ状態』を即座に開く」ための教材装置（3分クッキング方式）であり、上記の開発フロー（box → worktree → PR → merge）とは**別軸**。project（demo アプリ）本体をこの orphan 系列に置く。

- 新しい stage は必ず `scripts/internal/new-stage.sh` / `scripts/internal/new-stage.ps1` で作る（最初の stage は orphan、以降は前 stage を base に分岐）。素の `git worktree add` や `git checkout --orphan` で代替しない
- stage ブランチに講義進行用ファイル（README.md / CLAUDE.md / rules/ / scripts/ / stages/ / slides/）を混入させない（orphan 分離の維持）
- stage の project ドキュメント（project 自身の README 等）や**コード中のコメント**には **project（demo アプリ）の動かし方だけ**を書く。playbook の host 運用手順（box→host の閲覧経路 = `dev.sh route` / Traefik 等、main の `scripts/` 前提のコマンド）を複製しない（drift は doc だけでなく `.ts` 等のコメントにも潜るので、点検は `*.md` に限定しない）。stage は凍結 orphan で main と履歴を共有しないため、複製した手順は main の進化（例: `parallel.sh` → `dev.sh route`）に追従できず **drift** する（drift は merge/rebase でも直せない: 共有ファイル・共有履歴が無いため）。box→host の閲覧手順のような全 stage 共通の playbook 関心事は main 側に単一ソースで置く。project の部分は前 stage を base にした fork 連鎖で次 stage に伝播する（`new-stage.sh <new> <prev>`）
- 命名: ブランチは `stage/NN-<slug>`、worktree は `.worktrees/<NN-slug>/`
- worktree の削除は `git worktree remove`（手動 rm は stale 登録が残る）。clone 直後の未展開時は `bash scripts/internal/setup-worktrees.sh`（Windows: `scripts/internal/setup-worktrees.ps1`）で既存 `stage/*` を `.worktrees/` 配下に展開する
- **stage は凍結 checkpoint なので同期機構を持たない（意図的な不在）**。(a) main を stage に rebase/merge しない（別軸。main 側資産の drift は「複製しない・単一ソースを main に置く」で防ぐのであって同期では防がない。前項参照）。(b) stage 間も自動同期しない。前方伝播は `new-stage.sh <new> <prev>` の**作成時 fork 連鎖だけ**で、上流 stage を後から直して下流へ流す retroactive な仕組みは持たない。常時/自動同期は checkpoint を動く標的にして 3分クッキングの再現性を壊すため、入れない。遡及修正の伝播が実際に痛みになったら、CI 常駐ではなく**作業時に明示で叩く restack ヘルパー**（隣接 stage は fork 連鎖で履歴を共有するので `git rebase --onto` で載せ替え可能）として足す
...
講義スライドは**フェーズ単位**で main の `slides/<NN-slug>.html` に置く（壁打ち / 設計 / 実装 / 仕上げ(並列 issue 処理) / 運用保守・バグ修正 の 5 枚。スライドは状態でなくフェーズに対応するので stage の checkpoint 数とは一致しない。講義資料なので stage ブランチには入れない）。詳細は [docs/instructor.md](docs/instructor.md)「スライド」「ステージ (checkpoint 連鎖)」参照。

- reveal.js を CDN から読み込む単一の自己完結 HTML。中身は `<textarea>` 内の markdown 箇条書きで `---` でスライドを区切る
- CDN は固定バージョン + SRI（`integrity`）で読む。reveal.js のバージョンを上げる時は `integrity` ハッシュも再計算する（不一致だとブラウザがリソースをブロックして無言で壊れる）
- 新規作成は `slides/template.html` をコピーして中身を埋める（HTML 雛形は触らず markdown だけ編集）
- スライドの中身（箇条書き）は人間が書く。agent は雛形 `slides/template.html` を保守し、各フェーズの中身は埋めない
