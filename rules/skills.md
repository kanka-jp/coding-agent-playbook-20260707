...
| `test-sweep` | **leaf** | pre-PR sweep。新規追加 test ブロックを [rules/test-bloat-defense.md](test-bloat-defense.md) 規範で判定 → 違反テーブル提示 → ユーザー承認後 Edit で修正。`comment-sweep` / `co-evolve-check` / `extension-bloat-sweep` と並走可能な 4 本目の sweep |
| `fact-vs-speculation` | **leaf** | 調査結果・原因推定・コードレビュー提示時に、確認済みの事実と推測を書き分けさせる出力規範。外部ツール依存なしの pure prompt skill |
| `verify` | **leaf** | 実装後の動作検証。diff から検証項目を自動生成し、dev server 自動起動 + Chrome DevTools MCP / HTTP で動作確認（mode=local/deploy）。`manual-verify` / `refactor-check` と補完関係（使い分けは各 skill 内「使い分け」表参照） |
| `manual-verify` | **leaf** | PR の user-visible な変更を人間が手で確認するためのウォークスルーを生成。Chrome DevTools MCP でライブ画面に赤枠 overlay を注入し、base/PR を該当箇所ごとの 3 タブ群で並べる。最終判断は人間が行う |
| `refactor-check` | **leaf** | リファクタ PR の base/PR 間で `getComputedStyle` fingerprint を機械的に差分し、regression 候補を報告。真のリファクタなら差分ゼロになるはずという原理 |
| `mark-generated-viewed` | **leaf** | GitHub PR の自動生成ファイル（lockfile / protobuf / orval 等）を検出し Files changed タブで viewed にマーク。手書きファイルだけをレビュー対象に残す |
